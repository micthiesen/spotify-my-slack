...
module.exports.updateLoop = async function(userId) {
  ...
  setTimeout(() => {
    module.exports.updateLoop(userId);
  }, interval || process.env.UPDATE_LOOP_DEFAULT_INTERVAL);
};
...
const performUpdate = async (user, resolve) => {
  ...
  try {
    ...
    if (err.headers && err.headers["retry-after"]) {
      ...
      console.warn(`401 response received from Spotify. Ignoring...`);
      return resolve(["failure", null]);
    } else {
      console.warn(
        `Retrieving Spotify player state failed for user ${user.id}:`,
        err
      );
      return resolve(["failure", null]);
    }
  ...
  try {
    ...
    if (err.data && err.data.error === "token_revoked") {
      ...
      console.warn(
        `Updating Slack status failed for user ${user.id} (emoji ${statusEmoji}):`,
        err
      );
      return resolve(["failure", null]);
    }
...
const setUserStatus = async (user, spotifyItem) => {
  ...
  var statusText = `${spotifyItem.name} ${spotify.getArtistString(
    spotifyItem
  )}`;
  ...
  if (!user.statusSetLastTime) {
    await user.update({ statusSetLastTime: true });
  }
};
